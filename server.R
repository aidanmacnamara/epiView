...
shinyServer(function(input, output) {
  ...
  # # David's code
  # tensorTab("TV",input,output,ui=F,greetings="bore dda",gc=global_choice(),david=D(),
  #           trigger=input$do_global)
  # 
  # D=reactive({
  #   str(global_choice())
  # })
  ...
  global_plot <- reactive({
    cat("enter global_plot\n")
    
    gc = global_choice()
    a_gc <<- gc
    save(gc,file="try.rda")
